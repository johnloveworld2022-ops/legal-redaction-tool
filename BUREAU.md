...
- **The decision log is the source of truth.** `canonical` is a **projection** of a logged
  ...
  approval backs. Never hand-edit `canon/_log.jsonl`; it is append-only, and its hash chain detects naive edits
...
- **Mechanical, code-owned** (never hand-write) — run the bundled press,
  `node "${CLAUDE_PLUGIN_ROOT}/press/bin/gazette.mjs" <verb> --dir canon`. **Invoke it that
